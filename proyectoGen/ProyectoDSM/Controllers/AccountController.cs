    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                ...
                case SignInStatus.Success:
                    ...
                    IList<UsuarioEN> listaUsuarios = usu.DameUsuarioPorEmail(model.Email);
                    if (listaUsuarios.Count > 0)
                    {
                        Session["Usuario"] = listaUsuarios[0];

                        IList<SesionEN> listaSesiones = sesionCEN.DameSesionesUsuario(listaUsuarios[0].Id);
                        if (listaSesiones.Count == 0)
                        {
                            Session["IdSesion"] = sesionCEN.New_(DateTime.Now, listaUsuarios[0].Id);
                        }
                        else
                        {
                            SesionCP sesionCP = new SesionCP();
                            ...
                            foreach (SesionEN ped in listaSesiones)
                            {
                                ...
                                //Como hago que me cierre la sesion antigua o que no me deje loguearme
                                //sesionCP.CerrarSesion(ped.Id, listaUsuarios[0].Id);
        ...
        public async Task<ActionResult> Register(UsuarioViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    SesionCAD sesionCAD = new SesionCAD();
                    SesionCEN sesionCEN = new SesionCEN(sesionCAD);
                    Session["IdSesion"] = sesionCEN.New_(DateTime.Now, usuarioEN.Id);
        ...
        public ActionResult LogOff()
        {
            ...
            if(Session["IdSesion"]!= null)
            {
                ...
                int idSesion = (int)Session["idSesion"];
                UsuarioEN usuario = (UsuarioEN)Session["Usuario"];
                sesionCP.CerrarSesion(idSesion, usuario.Id);
            }
