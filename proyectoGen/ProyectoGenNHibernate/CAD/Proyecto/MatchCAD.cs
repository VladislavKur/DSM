...
public System.Collections.Generic.IList<ProyectoGenNHibernate.EN.Proyecto.UsuarioEN> FiltrarPorNombre (string p_nombre)
{
        ...
        {
                SessionInitializeTransaction ();
                //String sql = @"FROM MatchEN self where SELECT usu FROM MatchEN as ma1 inner join ma1.Usuario_emisor as usu WHERE usu.Nombre  like  '%'+ :p_nombre +  '%'";
...
public void GestionarMatch (MatchEN match)
{
        try
        {
                SessionInitializeTransaction ();
